...
export default function WaitlistReport() {
  ...
  const exportToExcel = () => {
    ...
    // Build rows matching template column order exactly:
    ...
    const data = waitlist.map(entry => {
      ...
      return {
        ...
        'City': entry.city || '',
        'State/Province': entry.state_province || '',
    ...
    const worksheet = XLSX.utils.json_to_sheet(data, {
      header: [
        'Order', 'Contact Name', 'Date Application Received',
        'Sponsor #1', 'Sponsor #2', 'City', 'State/Province',
        'Postal Code', 'Email', 'Phone', 'Street Address',
        'Days Waiting', 'Status'
      ]
    });

    // Format date column (C) as mm-dd-yy to match template
    const range = XLSX.utils.decode_range(worksheet['!ref']);
    for (let r = 1; r <= range.e.r; r++) {
      const cellAddr = XLSX.utils.encode_cell({ r, c: 2 });
      if (worksheet[cellAddr] && worksheet[cellAddr].v) {
        worksheet[cellAddr].t = 'd';
        worksheet[cellAddr].z = 'mm-dd-yy';
      }
    }
    ...
    worksheet['!cols'] = [
      { wch: 8 },   // Order
      { wch: 27 },  // Contact Name
      { wch: 24 },  // Date Application Received
      { wch: 26 },  // Sponsor #1
      { wch: 26 },  // Sponsor #2
      { wch: 24 },  // City
      ...
      { wch: 41 },  // Email
      { wch: 20 },  // Phone
      { wch: 32 },  // Street Address
      { wch: 20 },  // Days Waiting
      { wch: 12 },  // Status
    ];

    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, `waitlist_${dateStr}`);
    XLSX.writeFile(workbook, `waitlist_${dateStr}.xlsx`);
  };
